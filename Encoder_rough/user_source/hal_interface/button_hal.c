...
void button_init(void)
{
	...
	HAL_NVIC_SetPriority(EXTI2_TSC_IRQn, 3, 4); // Set the priority according to the application needs
	HAL_NVIC_EnableIRQ(EXTI2_TSC_IRQn); // Enable (un-mask) the EXTI0 interrupt in the NVIC
	count = 0;
}

int button_pressed(button b)
{
	//int ret = button_pressed_flag; 
		// Atomically save a copy of the button flag
	//button_pressed_flag = 0; 
		// Zero it atomically before returning
...
{
	HAL_GPIO_EXTI_IRQHandler(GPIO_PIN_2); 	// This function will first clear the IRQ on EXTI0
...
{
	if(GPIO_Pin == GPIO_PIN_2) // This means we're sure we've gotten an EXTI event on pin 0
	{
		if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_2) == GPIO_PIN_SET) // If the pin is high now, it must've been a
																// RISING edge we captured
																// I.e. the button was PRESSED
		{
			button_pressed_flag++;

			//count++; // Set the flag since we've detected a button press event
		}
	}
}